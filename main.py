...
def generate_authkey() -> str:
    authkey_list[authkey := f"authkey_{''.join(random.sample('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890', 8))}"] = time.time()
    return authkey
...
def login() -> bool:
    data = {
        ...
        "s": hashlib.sha1(f"/WebApi/Users/Login?username={username}&password={password}{security_salt}&t={timestamp}".encode('utf-8')).hexdigest()
    }
...
def verify(authkey: str) -> dict:
    if check_authkey(authkey):
        req = session.post("https://gitblock.cn/WebApi/Comment/GetPage", headers=headers, data={"forType":"User","forId":config['account']['userid'],"pageIndex":"1","scrollToCommentId":""}).json()
        if req['pagedThreads']['item'] == []:
            if time.time() - last_login_time > 600:
                if login():
                    req = session.post("https://gitblock.cn/WebApi/Comment/GetPage", headers=headers, data={"forType":"User","forId":config['account']['userid'],"pageIndex":"1","scrollToCommentId":""}).json()
                else:
                    return {"vaild": False, "code": 500}
            else:
                return {"vaild": False, "code": 500}
        
    else:
        return {"vaild": False, "code": 403}
...
if __name__ == '__main__':
    ...
    print(f"Generated authkey: {authkey}")
    print(login())
